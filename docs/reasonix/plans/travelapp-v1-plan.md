...
- Deps: Slice 0
- Acceptance: Auth required; create with valid data appears in list; invalid (missing title, end<start) shows Zod error; RLS prevents seeing other user's trips (test with two mocked users).
...
- Deps: Slice 1
- Acceptance: Add item appears grouped by date; sorted by time; validation (activity required); itinerary scoped to trip; cascade not yet needed but FK exists.
...
- Deps: Slice 1,2
- Acceptance: Owner can edit/delete own trip/item; non-owner gets 403/empty (RLS test); delete trip cascades items; optimistic UI + confirm dialog.
...
- Deps: Slice 1-3
- Acceptance: All async boundaries have loading/error/empty; touch targets >=44px; keyboard nav works; lint/typecheck/build/test green.
